...
fuser -k ${LISTEN_PORT}/tcp && (echo "Killed existing process on port ${LISTEN_PORT}, waiting .."; sleep 1)
fuser -k -9 ${LISTEN_PORT}/tcp && (echo "Killed -9 existing process on port ${LISTEN_PORT}, waiting .."; sleep .3)
